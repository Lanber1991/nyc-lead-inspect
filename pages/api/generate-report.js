...
async function generateSectionSummaries(insp, labData) {
  ...
  const SYSTEM = `You are writing a professional Lead Paint Inspection & Risk Assessment report for a client — a building owner or property manager. Write in past tense, third person. Be specific about locations, readings, and what they mean in plain language. Do not invent data not present in the inputs. When analyzing photos, describe exactly what is visible and what it indicates about paint condition, deterioration, or lead hazard risk — but NEVER reference the photograph itself in your writing. Do not write phrases like "as shown in the photo," "the image shows," "as seen in the photograph," "as depicted," or any similar language that refers to a visual document. Instead, describe conditions as if they were directly observed by the inspector during the inspection (e.g., "deteriorated paint with chipping and peeling was observed on the window sill" not "the photo shows deteriorated paint"). Report any XRF readings, moisture meter values, or visual measurements as direct inspector findings, not as image descriptions.
